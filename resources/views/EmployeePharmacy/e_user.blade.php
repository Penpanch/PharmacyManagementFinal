...
@endsection
  ...
  <div class="grid_12">
    <ul id="q_nav">
      <li><a href="{{url('/homeEm')}}">หน้าหลัก</a></li>
      <li><a href="#">/</a></li>
      <li><a href="{{url('/userEm')}}"  class="active">ดูลูกค้า</a></li>
    </ul>
  ...
  <tbody>
    ...
    @foreach($users->where('role', 'user') as $user)
          ...
          <tr class="table-active">
            <td><a href='{{url("/userEm/show/{$user->id}")}}'' >{{ $user->code_user }}</a></td>
            ...
            <td>{{ $user->drug }}</td>
              ...
              <td>
              <a href='{{url("/userEm/update/{$user->id}")}}'' class="label label-success">Update</a>
              <a href='{{url("/userEm/delete/{$user->id}")}}'' class="label label-danger">Delete</a>
            </td>
